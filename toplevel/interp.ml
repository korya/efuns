...
external asm_getglobal : int -> My_Obj.t = "asm_getglobal" 
...
let install_module i =
  if i>= first_native_module && i<= last_native_module then
    caml_globals.(i) <- 
        (My_Obj.magic (asm_getglobal (i - first_native_module)))
    
  (*
  if i>= first_native_module && i<= last_native_module then
    begin
      let modul = (My_Obj.magic (asm_getglobal (i - first_native_module))) in
      modul
    end
  else
    repr caml_globals.(i)
            ...
            repr (f 0 arg1) *)
                let rec f nargs args arg =
                  if nargs = required then
                    ...
                    (My_Obj.magic f : int -> t list -> t -> t) (nargs+1)
                    (arg :: args)
                in
                repr (f 1 [arg1])
          );
...
(*  let args = Array.create (required+1) (repr 0) in
      ...
      | OpATOM  -> 
          ...
          accu := repr (My_Obj.new_block atom 0)
      | OpPUSHATOM0  -> 
          decr sp; stack.(!sp) <- !accu;
          accu := repr ([||])
      | OpPUSHATOM  ->
      ...
      | OpMAKEBLOCK  ->
          ...
          sp := !sp + wosize - 1;
(*          
          let tab = My_Obj.magic (My_Obj.new_block tag wosize) in
...
(*            
  ...
  *)
      ...
      | OpGETFLOATFIELD  -> 
          ...
          accu := repr (tab.(pos))
      | OpSETFIELD0  -> 
          (My_Obj.magic !accu : t array).(0) <- stack.(!sp);
          incr sp;
      | OpSETFIELD1  ->
          (My_Obj.magic !accu : t array).(1) <- stack.(!sp);
          incr sp;
      | OpSETFIELD2  ->
          (My_Obj.magic !accu : t array).(2) <- stack.(!sp);
          incr sp;
      | OpSETFIELD3  ->
          (My_Obj.magic !accu : t array).(3) <- stack.(!sp);
          incr sp;
      | OpSETFIELD ->
          incr pc;
...
(* dangerous if float array *)
          accu := repr (Array.length (My_Obj.magic !accu))
      | OpGETVECTITEM  ->
...
(* Avec la garantie de Jacques et Xavier (garder un oeil sur
          matching.ml) *)
          if Compat_run.isint (My_Obj.magic obj) then
            begin (* INT *)
      ...
      | OpPOPTRAP ->
          ...
          raise poptrap
      | OpRAISE ->
          raise (My_Obj.magic !accu);
      | OpCHECK_SIGNALS  -> ()
      | OpC_CALL1  -> 
          incr pc; let prim = unsigned code.(!pc) in
          extern_sp := !sp;
          
          accu := (My_Obj.magic caml_prims.(prim)) !accu
      | OpC_CALL2  ->
          let arg = stack.(!sp) in
          incr sp;
          incr pc; let prim = unsigned code.(!pc) in
          ...
          accu := (My_Obj.magic caml_prims.(prim)) !accu arg
      | OpC_CALL3  ->
          let arg1 = stack.(!sp) in
          let arg2 = stack.(!sp+1) in
          sp := !sp + 2;
          incr pc; let prim = unsigned code.(!pc) in
          extern_sp := !sp;
          accu := (My_Obj.magic caml_prims.(prim)) !accu arg1 arg2;
      ...
      | OpC_CALL4  ->
          ...
          incr pc; let prim = unsigned code.(!pc) in
          extern_sp := !sp;
          accu := (My_Obj.magic caml_prims.(prim)) !accu arg1 arg2 arg3
      | OpC_CALL5  ->
          let arg1 = stack.(!sp) in
          let arg2 = stack.(!sp+1) in
          let arg3 = stack.(!sp+2) in
          ...
          incr pc; let prim = unsigned code.(!pc) in
          extern_sp := !sp;
          accu := (My_Obj.magic caml_prims.(prim)) !accu arg1 arg2 arg3 arg4
      | OpC_CALLN  ->
          ...
          incr pc; let prim = unsigned code.(!pc) in
          extern_sp := !sp;
          accu := (My_Obj.magic caml_prims.(prim)) args  nargs
      ...
      | OpOFFSETREF  -> 
          incr pc; let offset = magic code.(!pc) in
          (My_Obj.magic (magic !accu)).(0) <- (My_Obj.magic (magic !accu)).(0) + offset;
          accu := repr 0;
      | OpGETMETHOD  ->
      ...
      | OpBREAK  ->
          assert false; (* Not implemented *)          
      | OpISINT -> 
          accu := My_Obj.repr (Compat_run.isint (My_Obj.magic !accu))
      |  OpBEQ ->
          if (magic code.(!pc+1): int) == (magic !accu: int) then
            let offset = magic code.(!pc+2) in
            pc := !pc + 1 + offset;
          else pc := !pc + 2
      |  OpBNEQ ->
          if (magic code.(!pc+1) : int) != (magic !accu : int) then
      ...
      |  OpUGEINT ->
          accu := repr ((magic !accu : int) >= magic stack.(!sp));
          incr sp;
      |  OpBULTINT -> (* how to use unsigned ?? *)
          if (magic code.(!pc+1) : int) < (magic !accu : int) then
            let offset = magic code.(!pc+2) in
            pc := !pc + 1 + offset;
          else pc := !pc + 2
      |  OpBUGEINT -> (* how to use unsigned ?? *)
          if (magic code.(!pc+1) : int) >= (magic !accu : int) then
            let offset = magic code.(!pc+2) in
            pc := !pc + 1 + offset;
          else pc := !pc + 2
...
let insert_one name (f : My_Obj.t) = Hashtbl.add prims name f


let _ =
  List.iter (fun (name, func) ->
      insert_one name func) Compat_run.Externals.caml_primitives
...
external set_meta:
   My_Obj.t array * (string -> int -> 'a -> My_Obj.t) * My_Obj.t array
   -> unit = "set_meta"      
