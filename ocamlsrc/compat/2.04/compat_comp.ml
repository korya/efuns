...
(* val printlambda_lambda : Lambda.lambda -> unit *)  
let printlambda_lambda format lam = Printlambda.lambda lam
let printinstr_instrlist format init_code = Printinstr.instrlist init_code
let printtyp_ident format id = 
  Printtyp.ident id
let primitive_print_description format p =
  Primitive.print_description p
let printtyp_type_scheme format t = 
 Printtyp.type_scheme t

  
  
let print_value env obj format typ = Printval.print_value env obj typ
let printtyp_type_declaration id format decl =
  Printtyp.type_declaration id decl
let  printtyp_exception_declaration id format decl =
  Printtyp.exception_declaration id decl
let printtyp_modtype format mty = Printtyp.modtype mty
let printtyp_modtype_declaration id format decl =
    Printtyp.modtype_declaration id decl
let printtyp_class_declaration id format decl =
  Printtyp.class_declaration id decl
let printtyp_cltype_declaration id format decl =
  Printtyp.cltype_declaration id decl
  
let errors_report_error format x =  Errors.report_error x
let compile_implementation format f =
  Compile.implementation f
