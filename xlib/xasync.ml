...
let createWindow dpy window parent x y dx dy depth wa_class visual
    border_width swa =
  sendRequest dpy 
...
let createCursor dpy cursor src mask fred fgreen fblue bred bgreen bblue x y =
  sendRequest dpy 
    (createCursorReq cursor src mask fred fgreen fblue bred bgreen bblue x y)

let createGlyphCursor dpy cursor src mask src_char mask_char 
...
let createGC  dpy gc window arg =
  sendRequest dpy
...
let createPixmap dpy pixmap window dx dy depth =
  sendRequest dpy
