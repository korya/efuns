...
let install_module i =
  if i>= first_native_module && i<= last_native_module then
    caml_globals.(i) <- 
      (Obj.magic (asm_getglobal (i - first_native_module)))
  ...
  *)    
          ...
          | OpGRAB  ->
              ...
              (match required with
                ...
                | _ ->
                    let args = Array.create (required+1) unit in
                    let rec f n arg =
                      args.(n) <- arg;
                      if n = required then
                        let sp = extern_sp in
          ...
          | OpCLOSURE  ->
              ...
              if code.(goto) = opGRAB then
                ...
                repr (fun arg -> 
                    ...
                    let v = interp stack code goto unit clos recenv in
                    sp := trapsp;
                    v);
          ...
          | OpCLOSUREREC  -> 
              ...
              for i = 0 to nfuncs - 1 do
                incr pc; 
                let goto = oldpc + magic code.(!pc) in
                ...
                let fi =               
                  if code.(goto) = opGRAB then
                    ...
                    repr (fun arg ->
                        ...
                        let trapsp = !sp in              
                        decr sp;stack.(!sp) <- arg;
                        let v = interp stack code goto unit env recenv in
                        sp := trapsp;
                        v)
                in
                decr sp; stack.(!sp) <- repr fi;
                clos.(i*2) <- repr fi
              done
          ...
          | OpSETFIELD0  -> 
              ...
              (Obj.magic !accu: t array).(0) <- arg
          | OpSETFIELD1  ->
              let arg = stack.(!sp) in incr sp;
              (Obj.magic !accu: t array).(1) <- arg
          | OpSETFIELD2  ->
              let arg = stack.(!sp) in incr sp;
              (Obj.magic !accu: t array).(2) <- arg
          | OpSETFIELD3  ->
              let arg = stack.(!sp) in incr sp;
              (Obj.magic !accu: t array).(3) <- arg
          | OpSETFIELD ->
              let arg = stack.(!sp) in incr sp;
              incr pc; let pos = unsigned code.(!pc) in
              (magic !accu: t array).(pos) <-  arg
          | OpSETFLOATFIELD  ->
              let arg = stack.(!sp) in incr sp;
              incr pc; let pos = unsigned code.(!pc) in
              let tab = (magic !accu : float array) in
...
(*              
          ...
          *)
          ...
          | OpEQ  ->
              let arg = magic stack.(!sp) in incr sp;
              accu := repr ((magic !accu: int) == arg)
          | OpNEQ  ->
              let arg = magic stack.(!sp) in incr sp;
              accu := repr (not ((magic !accu: int) == arg))
          | OpLTINT  ->
              let arg = magic stack.(!sp) in incr sp;
              accu := repr ((magic !accu:int) < arg)
          | OpLEINT  ->
