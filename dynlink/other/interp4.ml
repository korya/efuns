...
let global_stack = Array.create 10000 (repr 0)
...
let rec interp (code: int array) pc env recenv accu =
...
*)    
      ...
      | OpGRAB  ->
          ...
          let call_interp = interp code goto env recenv in
          accu :=
          (match required with
              1 -> repr
                  (fun arg2 ->
                    ...
                    stack.(!sp+1) <- arg2;
                    stack.(!sp) <- arg1;
                    let v = magic (call_interp  (repr 0)) in
                    sp := trapsp;
                    v
                )
            | 2 -> repr
...
*)  
          ...
          if code.(goto) = opGRAB then
            ...
            repr (fun arg -> 
                ...
                let v = interp code goto clos recenv (repr 0) in
                sp := trapsp;
                v);
  ...
  let stack = (magic global_stack: t array) in
                    let trapsp = !sp in              
                    decr sp;stack.(!sp) <- arg;
                    let v = interp code goto env recenv (repr 0) in
                    sp := trapsp;
              v)
            in
...
(*            
  ...
  *)
      ...
      | OpGETFLOATFIELD  -> 
          ...
          accu := repr (tab.(pos))
      | OpSETFIELD0  -> 
          (Obj.magic !accu: t array).(0) <- stack.(!sp);
          incr sp;
      | OpSETFIELD1  ->
          (Obj.magic !accu: t array).(1) <- stack.(!sp);
          incr sp;
      | OpSETFIELD2  ->
          (Obj.magic !accu: t array).(2) <- stack.(!sp);
          incr sp;
      | OpSETFIELD3  ->
          (Obj.magic !accu: t array).(3) <- stack.(!sp);
          incr sp;
      | OpSETFIELD ->
          incr pc;
...
(* dangerous if float array *)
      ...
      | OpPUSHTRAP -> 
          ...
          begin
            try
              extern_sp := !sp;
              accu := interp code (!pc+1) env recenv !accu;
            with
              PopTrap -> 
                pc := !poppc; 
                accu := !popacc; 
